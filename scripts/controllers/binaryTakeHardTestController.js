binaryTestApp.controller('binaryTakeHardTestController', ["$scope", "$location", "_dataContext", "_toast", function ($scope, $location, context, _toast) {
    ...
    var row1Answer, row5Answer;
    var correct = 0;
    var correctArray;
    $scope.row1Summary = $scope.row2Summary = $scope.row3Summary = $scope.row4Summary = $scope.row5Summary = 0;
    $scope.row1ExpectedAnswer = 216; //11011000//
    $scope.row2ExpectedAnswer = 85;  //01010101//
    $scope.row3ExpectedAnswer = 255; //11111111//
    $scope.row4ExpectedAnswer = 10;  //00001010//
    $scope.row5ExpectedAnswer = 193; //11000001//
    $scope.correctArray = [];
    $scope.row1digit1 = false;//0
    $scope.row1digit2 = false;//0
    $scope.row1digit3 = false;//0
    $scope.row1digit4 = true;//1
    $scope.row1digit5 = true;//1
    $scope.row1digit6 = false;//0
    $scope.row1digit7 = true;//1
    $scope.row1digit8 = true;//1
    ...
    $scope.row5digit1 = true;
    $scope.row5digit2 = $scope.row5digit3 = $scope.row5digit4 = $scope.row5digit5 = $scope.row5digit6 = false;
    $scope.row5digit7 = $scope.row5digit8 = true;
    $scope.selectedClass = false;
    $scope.row5Answer = $scope.row1Answer = 0;

    $(document).ready(function () {
        $('.tooltipped').tooltip({ delay: 50 });
        _toast.info("Switching things up this time! Good luck!");
        $('#correctAnswersModal').modal();
         Materialize.updateTextFields(); //Initialise text fields
    });
    ...
    $scope.checkAnswers = function () {
        ...
        if ($scope.row1Answer == $scope.row1ExpectedAnswer) {
            correct++;
        }
        ...
        if ($scope.row5Answer == $scope.row5ExpectedAnswer) {
            correct++;
        }
